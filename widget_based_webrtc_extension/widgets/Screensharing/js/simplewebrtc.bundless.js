...
console.log(" screensharing instialization "+ screenSharing);
...
LocalMedia.prototype.startScreenShare = function (cb) {
    var self = this;
            ...
            self.localScreens.push(ssstream);
            ssstream.onended = function () {
                var idx = self.localScreens.indexOf(ssstream);
                if (idx > -1) {
                    self.localScreens.splice(idx, 1);
                }
                self.emit('localScreenStopped', ssstream);
            };
            self.emit('localScreen', ssstream);

        // enable the callback
        if (cb) {
            return cb(err, ssstream);
        }
...
function gotStream(stream) {
 ...
 // var video = document.querySelector("video");
  var video=document.getElementById("screesharingvideo");
  video.src = URL.createObjectURL(stream);
  localstream = stream;
  stream.onended = function() { console.log("Ended"); };

  ssstream=stream;
  // methods pickked from simplewebrtc under localscreensadded for screnshare 
  // webrtc.ss(stream);
  webrtc.getLocalScreen();
  webrtc.shareScreen();

  console.log("called webrtc.getlocal screen and wenrtc . share screen ");
}

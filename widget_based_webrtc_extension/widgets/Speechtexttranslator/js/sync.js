...
$('#text2speechCheckbox').change(function() {
   if($(this).is(":checked")) {
      //'checked' event code
      console.log("Check box event : read out the messages ");
      text2speechflag=true;
      return;
   }
  console.log(" dont read out the messages ");
  text2speechflag=false;
   //'unchecked' event code
});


/* var checkbox = document.getElementById ("text2speechCheckbox");
...
function texttospeech(textinput){
    ...
    console.log(" Text to speech of ",textinput);
...
if ('webkitSpeechRecognition' in window) {
  ...
  recognition.onresult = function(event) {
    ...
    console.log(" Speech to text of ",final_span.innerHTML);
    ...
    if(mytext!='' && mytext!=null){
      ...
      if(previous==null){
        console.log(" Previous assignmed now value for the firstb time ");
        ...
        if(previous!=now){
          console.log(" Previous : ", previous , " does not matches now ", now );
          sendmessagedictatedtext(mytext);
          final_transcript = '';
          final_span.innerHTML = '';
          interim_span.innerHTML = '';
         // $("#MessageBox").val('');
          ...
          console.log(" Previous : ", previous , " Matches now ", now );
          final_transcript = '';
          final_span.innerHTML = '';
          interim_span.innerHTML = '';
          //$("#MessageBox").val('');
...
$("#MessageBox").keyup(function(event){
    if(event.keyCode == 13){
        console.log("enter pressed for messages ");
...
function addMessageLog(who, msg){
  //alert(" Other ", who , msg);
    ...
    //do not add the previous log , write to text2speechDiv in messageHistoryBox
   /* showDiv("textorignaldiv");*/
    //showDiv("text2speechdiv");

    //create a new text area with every incoming message
    // yext area should resemble
    /*
    <textarea disabled class="messageHistory" rows="3" cols="60" id="MessageHistoryBox" style="overflow: hidden; border: none"></textarea>
    */
    if(who=="Other"){
...
function addMessageLangLog(who, msg){
    ...
    // do not add the previous log , instead display the content like a subtitle 
    //showDiv("text2speechdiv");
    //$('#MessageLangHistoryBox').text(msg);

    if(who=="Other"){
     // alert(" add msg lang lon", msg);
         var textArea = '<textarea disabled class="messageHistoryOther" rows="2" cols="90" style="overflow: hidden;  border: none">'+msg+'</textarea>'; 
       // textArea.text(msg);
        $('#text2speechdiv').append('<div class="message-other">'+textArea+'</div>');
       // $('#text2speechdiv').append('</br></br>'); 
...
/*------------------------background noise ----------------*/
// on windowon load , lower the background noise from main video element
//var parentpeervideoSource=window.parent.document.getElementById("localVideo");
...
document.getElementById("playhal").addEventListener("click", function(){
    ...
    if(halbuttonstatus=="stopped"){
     ...
     // document.querySelector("[data-func='stop']").disabled = false;
      ...
      //send the message accumulated to peers sisde 
      // $("#send_button").click();
...
function fetchstream(audiostream){
  ...
  console.log(" stream obtained on plugin page ");
}

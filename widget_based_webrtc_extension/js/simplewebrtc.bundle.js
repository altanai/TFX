...
//commenting this to prevent the stun from overriding my Xirsys and falling back to google stun 
    ...
    this.webrtc.on('localScreen', function (stream) {
        console.log(" webrtc on localscreen");
...
//added if else for room full by atanal for redis db at signalling server
        if (err) {
             console.log("~~~~~~~ error joiing a room " );
            self.emit('error', err);
        } else {
...
SimpleWebRTC.prototype.startLocalVideo = function () {
    ...
    this.webrtc.startLocalMedia(this.config.media, function (err, stream) {
        if (err) {
            ...
            console.log("seeting value for broplugselfStream : "+ broplugselfStream);
...
SimpleWebRTC.prototype.shareScreen = function (cb) {
    console.log(" simplewebrtc -----------share screen");
    this.webrtc.startScreenShare(cb);
};

SimpleWebRTC.prototype.getLocalScreen = function () {
    console.log(" simplewebrtc -----------get screen");
    return this.webrtc.localScreen;
};

SimpleWebRTC.prototype.stopScreenShare = function () {
    console.log(" simplewebrtc -----------stop screen");
...
SimpleWebRTC.prototype.testReadiness = function () {
    ...
    if (this.webrtc.localStream && this.sessionReady) {
        console.log(" test self rediness ------", self.connection.socket.sessionid);
...
SimpleWebRTC.prototype.createRoom = function (name, cb) {
    if (arguments.length === 2) {
        console.log("~~~~~~~  args 2 :  create room ", name );
        this.connection.emit('create', name, cb);
    } else {
        console.log("~~~~~~~ args 1 : create room name ", name );
...
SimpleWebRTC.prototype.sendFile = function () {
    console.log(" function for file sharing ");
...
Peer.prototype.send = function (messageType, payload) {
    ...
    console.log("Message Send || Type "+ message.type );
...
function LocalMedia(opts) {
    ...
    console.log(" Screen sharing support "+ this.screenSharingSupport);
...
LocalMedia.prototype.start = function (mediaConstraints, cb) {
    ...
    getUserMedia(constraints, function (err, stream) {
        ...
        //altanai added teh else 
        else{
            console.log(" Error in local media capture ");
...
LocalMedia.prototype.startScreenShare = function (cb) {
    ...
    getScreenMedia(function (err, stream) {
        ...
        console.log("Local stream ------ start screenshare ");
        ...
        if (!err) {
            ...
            console.log(" Local media error for screes share "+ err);
        }
        // enable the callback
        if (cb) {
            return cb(err, stream);
...
LocalMedia.prototype.stopScreenShare = function (stream) {
    console.log("Local stream ------ stop screenshare ");
    if (stream) {
        stream.stop();
    } else {
...
PeerConnection.prototype.getStats = function (cb) {
    console.log(" Getstatst under PeerConnection.prototype.getStats");
...
module.exports = function (constraints, cb) {
    ...
    if (window.navigator.userAgent.match('Chrome')) {
        ...
        if (chromever >= 26 && chromever <= maxver) {
            ...
            console.log( " windows chromw version above 34 . check for timeout for screen share ");    
            ...
            console.log(" sending to window --> type : get screen  with id ",pending);
...
window.addEventListener('message', function (event) {
    ...
    console.log(" window event listenre in simple webrtc ", event);
    

    if (event.origin != window.location.origin) {
        console.log("evenet origin is not equal to window location origin .. wateveer that means ");
        return;
    }
    
    if (event.data.type == 'gotScreen' && cache[event.data.id]) {
        
        console.log("got screen event listner ");
        ...
        {
            console.log(" Source ID : ",event.data.sourceId);
            console.log(" Setting constraints for desktopn sharing ");
            ...
            getUserMedia(constraints, callback);
            console.log(" just called get user media for chorme media source desktop ",constraints);
        }
    } else if (event.data.type == 'getScreenPending') {
        console.log("screen pending ");
...
exports.toMediaSDP = function (content) {
    ...
    if (desc.bandwidth && desc.bandwidth.type && desc.bandwidth.bandwidth) {
        console.log(" Bandwidth "+ desc.bandwidth + '  '+ desc.bandwidth.type + '  '+ desc.bandwidth.bandwidth);
...
TraceablePeerConnection.prototype.getStats = function (callback, errback) {
    console.log(" TraceablePeerConnection.prototype.getStats ");
...
//     console.log(" Inside Simplewebrtc : play Audio ");
//     var self = this;
...
//     console.log(" Inside Simplewebrtc : play video ");
//     var self = this;
...
SimpleWebRTC.prototype.playAudio = function (onSucess, onError) {
    ...
    this.webrtc.startLocalMedia({ "audio": true, "video": false }, function (err, stream) {
        if (err) {
            ...
            console.log(" no error in local webrtc audio");
...
SimpleWebRTC.prototype.playVideo = function (onSucess, onError) {
    ...
    this.webrtc.startLocalMedia({ "audio": false, "video": true }, function (err, stream) {
        if (err) {
            console.log(" error in local webrtc video"+ err);
            onWebRTCError("video",err);
        } else {
            console.log(" no error in local webrtc video");
...
SimpleWebRTC.prototype.sendControlPackets= function(msg, event , peer, opts){
    ...
    this.webrtc.peers.forEach(function (peer) {
        console.log(peer.enableDataChannels);
...
/*----------------------plugins access simpelwebrtc video audio streamms ------------------*/
  ...
  SimpleWebRTC.prototype.pluginRequestStream = function (pluginname) {
    ...
    console.log("callbackBroplugpluginRequestStream",pluginname);
...
/*------------------- plugins want to view stats from peercpncetion object -------*/
 ...
 SimpleWebRTC.prototype.passWebrtcStats = function () {
    ...
    console.log("get webrtc stats ",peerCoonectionObj.pc);
